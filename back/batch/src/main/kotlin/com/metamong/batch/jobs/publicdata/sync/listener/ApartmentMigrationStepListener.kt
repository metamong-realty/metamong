...
class ApartmentMigrationStepListener : StepExecutionListener {
    ...
    override fun afterStep(stepExecution: StepExecution): ExitStatus? {
        val stepName = stepExecution.stepName
        val duration =
            if (stepExecution.endTime != null && stepExecution.startTime != null) {
        ...
        if (skipCount > writeCount * 0.1) { // 스킵이 10% 이상
            logger.warn {
                "[$stepName] 스킵 건수가 높습니다: $skipCount (전체의 ${String.format(
                    "%.1f",
                    (skipCount.toDouble() / (writeCount + skipCount)) * 100,
                )}%)"
            }
        }
        ...
        if (rollbackCount > commitCount * 0.05) { // 롤백이 5% 이상
            logger.warn {
                "[$stepName] 롤백 건수가 높습니다: $rollbackCount (커밋의 ${String.format(
                    "%.1f",
                    (rollbackCount.toDouble() / commitCount) * 100,
                )}%)"
            }
        }
