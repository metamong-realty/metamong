---
description: "변경된 코드를 Gemini API로 전송하여 외부 AI 모델의 독립적 시각으로 코드 리뷰를 수행합니다. 사용자가 'Gemini 리뷰', '외부 리뷰', '다른 관점에서 리뷰'를 요청하거나, Claude 리뷰에 추가로 외부 검증이 필요할 때 사용합니다."
---
...
Bash로 `gcloud` CLI 설치 및 인증 상태를 확인하라:

```bash
gcloud auth print-access-token 2>&1 | head -1
```

토큰이 출력되지 않거나 에러가 발생하면 아래 안내를 출력하고 **즉시 종료**하라:
...
설정 방법:
  ...
  3. 프로젝트 설정: gcloud config set project YOUR_PROJECT_ID
```
